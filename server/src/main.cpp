...
void CMain::JSONUpdateThread(void *pUser)
{
	...
	while(gs_Running)
	{
		...
		for(int i = 0; i < NET_MAX_CLIENTS; i++)
		{
			...
			{
				str_format(pBuf, sizeof(aFileBuf) - (pBuf - aFileBuf), "{ \"ip\": \"%s\",\"url\": \"%s\",\"price\": \"%s\", \"name\": \"%s\", \"type\": \"%s\", \"host\": \"%s\", \"location\": \"%s\", \"online4\": false, \"online6\": false },\n",
					pClients[i].m_aIp, pClients[i].m_aUrl, pClients[i].m_aPrice, pClients[i].m_aName, pClients[i].m_aType, pClients[i].m_aHost, pClients[i].m_aLocation);
				pBuf += strlen(pBuf);
			}
		}
...
int CMain::ReadConfig()
{
	...
	if(rStart.type == json_array)
	{
		for(unsigned i = 0; i < rStart.u.array.length; i++)
		{
			...
			str_copy(Client(ID)->m_aIp, rStart[i]["ip"].u.string.ptr, sizeof(Client(ID)->m_aIp));
			str_copy(Client(ID)->m_aUrl, rStart[i]["url"].u.string.ptr, sizeof(Client(ID)->m_aUrl));
			str_copy(Client(ID)->m_aPrice, rStart[i]["price"].u.string.ptr, sizeof(Client(ID)->m_aPrice));
